...
class FirDataset(Dataset):
    def __init__(self, subset):
        ...
        self.datasetid_to_filepath = self.df.to_dict()['filepath']
        self.datasetid_to_class_id = self.df.to_dict()['class_id']
        print(self.df.to_dict()['class_id'])
    ...
    def __getitem__(self, item):
        ...
        #instance = Image.LOAD_TRUNCATED_IMAGES = True
    ...
    def index_subset(subset):
        ...
        # Quick first pass to find total for tqdm bar
        subset_len = 0
        for root, folders, files in os.walk(DATA_PATH + '/FIR/images_{}/'.format(subset)):
            subset_len += len([f for f in files if f.endswith('.png')])
